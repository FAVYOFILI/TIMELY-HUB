...
const RightBar: React.FC = () => { 
  ...
  const [newEvent, setNewEvent] = useState({ title: '', time: '09:00', type: 'class' as const });
  ...
  const getDaysInMonth = (): (Date | null)[] => {
    ...
    return [
      ...
      ...Array.from({ length: daysInMonth }, (_, i) => new Date(year, currentDate.getMonth(), i + 1))
    ];
  };

  const formatDateKey = (date: Date): string => date.toISOString().split('T')[0];
  const hasEvents = (date: Date): boolean => !!(events[formatDateKey(date)]?.length);
  ...
  return (
    <div className="bg-white shadow-sm border border-gray-100 p-3 w-[18%] text-xs">
      ...
      <div className="flex justify-between items-center mb-3">
        <div className="font-poppins font-semibold text-[#102844]">{month} {year}</div>
      ...
      <div className="grid grid-cols-7 gap-0.5 mb-3">
        {dayNames.map(day => (
          <div key={day} className="text-center font-semibold text-[#767278] py-1 text-[10px]">
            {day}
          </div>
        ))}
          ...
          day ? (
            <div
              ...
              className={`aspect-square flex items-center justify-center rounded text-[10px] cursor-pointer transition-all
                ...
                ${isSelected(day) ? 'bg-[#0D9165] text-white' : 'bg-gray-50 hover:bg-gray-100'}
              `}
            ...
            >
              <div className="flex flex-col items-center">
                ...
                {hasEvents(day) && (
                  <div className={`w-1 h-1 rounded-full mt-0.5 ${isSelected(day) ? 'bg-white' : 'bg-[#0D9165]'}`} />
                )}
              </div>
            </div>
      ...
      <div className="border-t border-gray-100 pt-2">
        ...
        {/* Events Container - Scroll ONLY when events exceed 3 items */}
        <div className={selectedEvents.length > 3 ? "max-h-[350px] overflow-y-auto" : ""}>
          {selectedEvents.map(event => (
            <div key={event.id} className="flex items-center gap-2 p-1 py-3 bg-gray-50 rounded mb-1">
              <div className={`w-2 h-2 rounded-full ${
                event.type === 'class' ? 'bg-blue-400' : 
                event.type === 'study' ? 'bg-green-400' : 'bg-amber-400'
              }`} />
              <span className="truncate flex-1 text-xs">{event.time} {event.title}</span>
            </div>
          ))}
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 backdrop-blur bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-4 w-full max-w-[280px] text-xs">
            <h3 className="font-poppins font-semibold text-[#102844] mb-3">Add Event</h3>
